...
export function ChatShell({ currentUserId }: { currentUserId: string }) {
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`chat-messages:${activeId ?? "idle"}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
        ...
        (payload: { new: Record<string, unknown> }) => {
          ...
          void loadConversations(activeId ?? undefined);
        }
      )
      .subscribe();
          ...
          {conversations.map((conversation) => (
            <button
              ...
              className={cn(
                ...
                conversation.id === activeId || conversation.unread_count > 0 ? "bg-panelSoft" : "hover:bg-panelSoft"
              )}
            >
              <UserAvatar
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {conversation.unread_count > 0 && (
                    <span className="ml-auto grid min-w-5 place-items-center rounded-full bg-danger px-1.5 py-0.5 text-[11px] font-black text-white">
                ...
                <p
                  className={cn(
                    "truncate text-xs",
                    conversation.unread_count > 0 ? "font-bold text-zinc-200" : "text-zinc-500"
                  )}
                >
