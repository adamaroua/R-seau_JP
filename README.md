...
- Vercel free tier
- Cloudflare Workers/Pages via OpenNext
- Supabase free tier
...
Pour verifier la version Cloudflare en local:
...
Ce projet utilise des routes API Next.js et ne doit pas etre deploye comme simple export statique.
Pour Cloudflare, utilise OpenNext:
...
Dans Cloudflare Pages/Workers, ajoute **toutes** les variables d'environnement **avant le build**
(dans Settings > Environment variables, branche Production). Les `NEXT_PUBLIC_*` sont injectees
au moment du build Next.js: sans elles, le deploiement peut echouer ou le site ne pourra pas se
...
- `NEXT_PUBLIC_SITE_URL` (ex: `https://jpzone.duckdns.org`)
- `SUPABASE_SECRET_KEY` (secret, jamais expose cote client)
- `CRON_SECRET` (secret)

Si tu deployes avec Wrangler:
...
Sur Cloudflare, remplace le domaine par ton domaine Cloudflare:

```bash
curl "https://ton-site.example/api/cron/cleanup?secret=TON_CRON_SECRET"
```
...
- Regarde regulierement les logs Cloudflare/Vercel du cron.
